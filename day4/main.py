...
def part1():
    ...
    for line in input.splitlines():
        rawline = line
        line, checksum = line.strip()[:-1].split("[")
        pieces = line.split("-")
        letters = "".join(pieces[:-1])
        sectorId = int(pieces[-1])
        counter = collections.Counter(letters)

        sorted_common = sorted(counter.most_common(), key=lambda x: (-x[1], x[0]))[:5]

        if (''.join([s[0] for s in sorted_common]) == checksum):
            words = rawline.split("[")[0].split("-")[:-1]
            new_words = []
            for w in words:
                new_words.append(caesarShift(w, sectorId))

            sentence = " ".join(new_words)
            if "pole" in sentence.lower():
                print sentence, sectorId
